<?php
    ...
    if($uid){
        $land= "SELECT *FROM dynamic WHERE uid='$uid' LIMIT $pageStart,$pageSize";
        $lanc="SELECT COUNT(*) FROM dynamic WHERE uid='$uid'";
        ...
        if($queryLand&&$queryLanc){
            ...
            while($resLand=mysql_fetch_array($queryLand)){
                ...
                $arr["data"][$i]["title"] = $resLand["title"];
                $arr["data"][$i]["describe"] = $resLand["describe"];
                $arr["data"][$i]["content"] = $resLand["content"];
                $arr["data"][$i]["img"] = $resLand["img"];
